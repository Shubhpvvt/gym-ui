...
export default function GymOwnerManagement() {
  ...
  /* LOAD OWNERS */
  useEffect(() => {
    axios.get("http://localhost:5000/api/gym-owners", {
  ...
  /* ADD OWNER */
  const addOwner = () => {
    console.log("ADD OWNER CLICKED"); // 👈 confirm click
    ...
    axios.post(
      "http://localhost:5000/api/gym-owners",
      form,
      { headers: { Authorization: `Bearer ${token}` } }
    )
    .then(res => {
  ...
  const deleteOwner = (id) => {
    axios.delete(`http://localhost:5000/api/gym-owners/${id}`, {
      headers: { Authorization: `Bearer ${token}` },
    })
    .then(() => {
  ...
  const updateOwner = () => {
    axios.put(
      `http://localhost:5000/api/gym-owners/${editOwner._id}`,
      editOwner,
      { headers: { Authorization: `Bearer ${token}` } }
    )
    .then(res => {
      setOwners(
        owners.map(o => o._id === editOwner._id ? res.data.owner : o)
      );
      setEditOwner(null);
    });
  };
        ...
        <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
          ...
          <button
            ...
            className="bg-slate-900 text-white px-5 py-2 rounded-md
                       hover:bg-slate-800 pointer-events-auto"
          >
            Add Owner
          </button>
        </div>
